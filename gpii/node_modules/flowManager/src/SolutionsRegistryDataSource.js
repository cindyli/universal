...
fluid.defaults("gpii.flowManager.solutionsRegistry.dataSource", {
    ...
    invokers: {
        ...
        getCurrentVersion: {
            funcName: "gpii.flowManager.solutionsRegistry.dataSource.getCurrentVersion",
...
/**
 * Return either a full solution registry metadata, or if an 'os' is provided, only the encoding version
 * for that os will be returned.
 *
 * @param {Object} that - The gpii.flowManager.solutionsRegistry.dataSource.
 * @param {Integer} os - If provided, the encoding version for that os will be returned.
 * @return {Object|Integer} Either a full solution registry metadata (Object), or if an 'os' is provided, only the
 * encoding version (Integer) will be returned.
 */
gpii.flowManager.solutionsRegistry.dataSource.getCurrentVersion = function (that, os) {
    if (os) {
        return that.fullSolutionsRegistry.metadata[os].encodingVersion;
    } else {
        return that.fullSolutionsRegistry.metadata;
    }
};
