...
fluid.defaults("gpii.flowManager.local", {
    ...
    distributeOptions: {
        ...
        "lifecycleManager.qssSettings": {
            record: "{that}.options.qssSettings",
            target: "{that gpii.lifecycleManager.loginRequest}.options.qssSettings"
        }
    },
    qssSettings: null,  // if not provided, populated at onCreate using QssSettingsLoader subcomponent
    components: {
        lifecycleManager: {
    ...
    events: {
        ...
        qssSettingsLoaded: null,
        readyToLoginNoUser: {
            events: {
                onListen: "{kettle.server}.events.onListen",
                qssSettingsLoaded: "qssSettingsLoaded"
            }
        },
        noUserLoggedIn: null,
    },
    listeners: {
        ...
        "onCreate.loadQssSettings": {
            listener: "gpii.flowManager.local.loadQssSettings",
            args: ["{that}"]
        },
        // login with "noUser" when GPII starts so that QSS is operable when no actual key is keyed in
        "readyToLoginNoUser.noUserLogin": {
            listener: "gpii.flowManager.local.noUserLogin",
...
gpii.flowManager.local.loadQssSettings = function (that) {
    if (!that.options.qssSettings) {
        that.options.qssSettings = that.qssSettingsLoader.get();
        that.events.qssSettingsLoaded.fire(that.options.qssSettings);
    }
};
...
gpii.flowManager.local.noUserLogin = function (lifecycleManager, noUserLoggedInEvent) {
