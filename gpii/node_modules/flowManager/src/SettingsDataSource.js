...
fluid.defaults("gpii.flowManager.settingsDataSource", {
    ...
    localClientCredentialFilePath: null,
    ...
    invokers: {
        get: {
            ...
            args: ["{that}", "{arguments}.0", "{arguments}.1", "{arguments}.2"]
                             // gpiiKey, device, clientCredentialType
        },
        set: {
            funcName: "gpii.flowManager.settingsDataSource.set",
            args: ["{that}", "{arguments}.0", "{arguments}.1", "{arguments}.2"]
                             // gpiiKey, preferences, clientCredentialType
        },
        save: {
 ...
 * @param {String} clientCredentialType - The client credential type.
 * @return {Promise} A promise whose resolved value is the user settings.
 */
gpii.flowManager.settingsDataSource.get = function (that, gpiiKey, device, clientCredentialType) {
    var accessTokenPromise = gpii.flowManager.settingsDataSource.findValidAccessToken(that, gpiiKey, clientCredentialType);
 ...
 */
gpii.flowManager.settingsDataSource.set = function (that, gpiiKey, preferences, clientCredentialType) {
    var accessTokenPromise = gpii.flowManager.settingsDataSource.findValidAccessToken(that, gpiiKey, clientCredentialType);
    var promiseTogo = fluid.promise();
    accessTokenPromise.then(function (accessToken) {
 ...
 */
gpii.flowManager.settingsDataSource.findValidAccessToken = function (that, gpiiKey, clientCredentialType) {
    ...
    if (!accessToken || !expiresIn || expiresIn < that.options.minAccessTokenLifeTimeInSecond) {
        var accessTokenPromise = that.accessRequester.getAccessToken(gpiiKey, clientCredentialType);
