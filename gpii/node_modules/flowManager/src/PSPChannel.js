...
gpii.pspChannel.findAppTermSchema = function (appSolution, requestSetting) {
    var schema;
    fluid.each(fluid.get(appSolution, "settingsHandlers"), function (settingsHandlerDef) {
        fluid.each(fluid.get(settingsHandlerDef, "supportedSettings"), function (settingsDef, settingsKey) {
            if (settingsKey === requestSetting && settingsDef.schema) {
                schema = settingsDef;
            }
        });
    });
        ...
        if (prefsKey.startsWith(prefApplicationPrefix)) {
            ...
            if (solution) {
                fluid.each(prefsVal, function (innerPrefsVal, innerPrefsKey) {
                    var settingSchema = gpii.pspChannel.findAppTermSchema(solution, innerPrefsKey);
                    var liveness = gpii.matchMakerFramework.utils.getLeastLiveness([ solution ], innerPrefsKey);
                    gpii.pspChannel.emitSettingControl(settingControls, settingSchema, innerPrefsVal, [prefsKey, innerPrefsKey], liveness, solution.name);
                });
            }
        } else {
