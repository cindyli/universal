...
(function () {
    ...
    fluid.defaults("gpii.snapshotter", {
        ...
        qssSupportedFileLocation: "%gpii-universal/testData/solutions/qssSupported.json5",
        model: {
            originalSettings: {}, // Holds original settings that QSS supports
            provenance: {}
        },
        events: {
    ...
    gpii.snapshotter.takeSnapshot = function (that, invokeSettingsHandlerGet, resolver, solutions) {
        var qssSupportedSolutions = JSON5.parse(fs.readFileSync(fluid.module.resolvePath(that.options.qssSupportedFileLocation), "utf-8"));
        ...
        promisesSequence.then(function (originalSettings) {
            fluid.log("=== Snapshotter success originalSettings: ", originalSettings);
            that.applier.change("originalSettings", originalSettings);

            // // Populate model.provenance based on the settings found
            // var provenance = {};
            // fluid.each(originalSettings, function (value, solutionId) {
