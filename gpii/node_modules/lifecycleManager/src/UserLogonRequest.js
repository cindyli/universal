...
fluid.defaults("gpii.lifecycleManager.loginRequest", {
    ...
    logonState: "login",
    qssSettings: null,   // distributed from gpii.flowManager.local
    components: {
        privateMatchMaker: {
            type: "gpii.lifecycleManager.privateMatchMaker",
            options: {
                gpiiKey: "{loginRequest}.options.gpiiKey",
                rawPreferences: "{loginRequest}.options.qssSettings"
            }
        }
    },
    listeners: {
        "onMatchDone.debug": {
            listener: "fluid.log",
            args: ["=== regular matchMaking: ", "{arguments}.0"],
            // listener: "gpii.lifecycleManager.loginRequest.privateMatchMaking",
            // args: ["{arguments}.0", "{that}.options.qssSettings", "{that}.privateMatchMaker"],
                   // payload
            priority: 110
        }
    }
...
gpii.lifecycleManager.loginRequest.getSnapshotSettings = function (preferences, qssSettings) {
    var userPrefsKeys = fluid.keys(preferences);
    return fluid.transform(qssSettings, function (value, key) {
        if (userPrefsKeys.indexOf(key) === -1) {
            return value;
        }
    });
};

gpii.lifecycleManager.loginRequest.privateMatchMaking = function (payload, qssSettings, privateMatchMaker) {
    // var gpiiKey = payload.gpiiKey,
    //     preferences = payload.preferences;
    //
    // var snapshotSettings = gpii.lifecycleManager.loginRequest.getSnapshotSettings(preferences, qssSettings);
    //
    // var privateMatchMakingPromise = privateMatchMaker.matchMaking(gpiiKey, snapshotSettings);
};
