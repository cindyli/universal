...
fluid.defaults("gpii.lifecycleManager.userLogonRequest", {
    ...
    clientCredentialType: null, // A client credential type: "secured" or "local"
...
gpii.lifecycleManager.userLogonRequest.handleRequest = function (that, lifecycleManager, logonState) {
    ...
    var clientCredentialType = that.options.clientCredentialType;
    ...
    if (logonState === "login") {
        that.loginUser(gpiiKey, clientCredentialType);
    }
...
gpii.lifecycleManager.processUserLogonRequestQueue = function (that, queue) {
    ...
    that.events.onUserLogonRequestReceived.fire(gradeName, item.gpiiKey, item.clientCredentialType, item.requestPromise);
