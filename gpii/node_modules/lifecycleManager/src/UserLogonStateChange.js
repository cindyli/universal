...
gpii.lifecycleManager.userLogonHandling.loginUser = function (that, lifecycleManager, gpiiKey, clientCredentialType) {
    gpii.lifecycleManager.userLogonHandling.updateLogonChangeModel(lifecycleManager, gpiiKey, "login", true);
    that.events.onGpiiKey.fire(gpiiKey, clientCredentialType);
};
...
fluid.defaults("gpii.lifecycleManager.userLogonHandling.stateChangeHandler", {
    ...
    invokers: {
        ...
        loginUser: {
            ...
            args: ["{that}", "{lifecycleManager}", "{arguments}.0", "{arguments}.1"]
        },
        handleResetGpiiKey: {
    ...
    listeners: {
        "onMatchDone": [{
            priority: 100,
            namespace: "updateActivePrefsSetName",
...
fluid.defaults("gpii.lifecycleManager.untrusted.stateChangeHandler", {
    ...
    listeners: {
        "onGpiiKey.setGpiiKey": {
            listener: "fluid.set",
            args: ["{that}", ["gpiiKey"], "{arguments}.0"]
        },
        "onGpiiKey.setClientCredentialType": {
            listener: "fluid.set",
            args: ["{that}", ["clientCredentialType"], "{arguments}.1"],
            priority: "after:setGpiiKey"
        },
        "onGpiiKey.getDeviceContext": {
            func: "{that}.getDeviceContext",
            priority: "after:setClientCredentialType"
        },
        "onDeviceContext.getSettings": "{that}.getSettings"
    }
});

gpii.lifecycleManager.untrusted.getSettings = function (settingsDataSource, gpiiKey, deviceReporterData, clientCredentialType, onMatchDone, onError) {
    ...
    var settings = gpiiKey === "noUser" ? fluid.promise().resolve(noUserSettings) : settingsDataSource.get(gpiiKey, deviceReporterData, clientCredentialType);

    settings.then(function (payload) {
        fluid.set(payload, ["clientCredentialType"], clientCredentialType);
        onMatchDone.fire(payload);
    }, onError.fire);
};
