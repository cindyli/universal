...
gpii.tests.userListener.insertTests = [
    ...
    // Already a device, added one with same GPII key (rare)
    {
        current: "gpiiKey1",
        device: "gpiiKey1",
        expect: "gpiiKey1"
...
/**
 * Perform the device add/remove tests - tests reading the GPII key file from the mount point, with the correct reaction.
 *
 * @param {Array} tests - An array of test data objects.
 * @param {Function} func - The function to test - gpii.userListeners.usb.readUSBGpiiKey or deviceRemoved.
...
gpii.tests.userListener.deviceMountTests = function (tests, func, createFiles, expectedNumOfAsserts) {
    ...
    var gpiiKeyFile = ".gpii-user-gpiiKey.txt";
    ...
    var devices = {};
    fluid.each([ "gpiiKey1", "gpiiKey2", "empty" ], function (name) {
        ...
        if (createFiles) {
            ...
            if (name !== "empty") {
                fs.writeFileSync(path.join(dir, gpiiKeyFile), name);
            }
        }
    });
    ...
    var testWork = fluid.transform(tests, function (test) {
        return function () {
            ...
            fluid.toPromise(p).then(
                function (value) {
                    jqUnit.assertEquals("gpiiKey should match the expected", test.expect, value);
                },
                function (value) {
                    if (test.expect) {
                        fluid.log(value);
                    }
                    jqUnit.assertNull("readUSBGpiiKey should reject", test.expect);
                }
...
jqUnit.asyncTest("USB listener - device inserted", function () {
    gpii.tests.userListener.deviceMountTests(
        gpii.tests.userListener.insertTests,  gpii.userListeners.usb.readUSBGpiiKey, true, 9);
});
