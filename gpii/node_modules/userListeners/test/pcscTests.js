...
jqUnit.asyncTest("PC/SC listener - mock card", function () {
    ...
    listener.events.onCardRejected.addListener(function () {
        ...
        jqUnit.assertNull("cards with a GPII key should succeed" + messageSuffix, currentTest.expect.text);

        jqUnit.assert("balance the numbers for jqUnit.expect");
        nextTest();
    });
    listener.events.onGpiiKeyArrive.addListener(function (that, gpiiKey) {
        jqUnit.assertNotEquals("listener trigger should only fire onGpiiKeyArrive when a card is presented"
            + messageSuffix, -1, testIndex);

        jqUnit.assertNotNull("cards with no GPII key should not succeed" + messageSuffix, currentTest.expect.text);
        jqUnit.assertEquals("gpiiKey should match expected" + messageSuffix, currentTest.expect.text, gpiiKey);
        nextTest();
    });
