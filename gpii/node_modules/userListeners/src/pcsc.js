...
/**
 * Monitors a card reader. When a card is presented to the reader, read it and raise the onGpiiKeyArrive event when
 * a GPII key is found.
 *
 * @param {Component} that The gpii.userListeners.pcsc instance.
    ...
    reader.on("status", function (status) {
        ...
        if (action === "presented") {
            ...
            gpii.userListeners.pcsc.readCard(that, reader, status.atr).then(function (tag) {
                that.currentGpiiKey = tag;
                that.events.onGpiiKeyArrive.fire(that, that.currentGpiiKey);
            }, function (err) {
                fluid.log(err);
                that.events.onCardRejected.fire(that);
            });
        } else if (action === "removed") {
            if (that.currentGpiiKey) {
                that.events.onGpiiKeyRemove.fire(that, that.currentGpiiKey);
                that.currentGpiiKey = null;
            }
        }
    });
